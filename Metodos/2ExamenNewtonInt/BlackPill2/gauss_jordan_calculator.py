...
def solve_resistive_circuit(R,VCC):
    """
    Solve the resistive circuit with given resistor values.
    
    Parameters:
    R: list/array of 7 values [R1, R2, R3, R4, R5, R6, VCC]
    ...
    vector = np.array([VCC, 0, 0, 0, 0, 0], dtype=float)
...
if __name__ == "__main__":
    ...
    try:
        ...
        VCC = float(input("VCC (V) [default 200]: ") or 200)
        R = [R1, R2, R3, R4, R5, R6, VCC]
        
    except ValueError:
        print("Invalid input, using default values")
        R = [100, 200, 300, 400, 500, 600, 200]

    # Solve using numpy
    print("\n" + "#" * 60)
    print("METHOD 1: NumPy Linear Solver (optimized)")
    print("#" * 60)
    currents_numpy = solve_resistive_circuit(R,VCC)
